...
func EntryAPI(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(r.RequestURI, "/api/entry/query") {
		...
		if err = json.Unmarshal(body, &filter); err == nil {
			response, _ := ownerGrpc.Query(context.Background(), &filter)
			respond(w, response)
			return
		}
	} else if strings.HasPrefix(r.RequestURI, "/api/file") {
		// TODO implement serving media file via http json request
		/*var entry Entry
		if err := json.Unmarshal(body, &entry); err == nil {
			response, _ := userOwnGrpc.Get(context.Background(), &request)
			respond(w, response)
			return
		}*/
	}
